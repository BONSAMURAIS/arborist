...
def get_metadata(rdf_base):
    ...
    metadata["activitytype"]["exiobase"] = dict(
        get_turtle_labels(
            rdf_base / "activitytype" / "exiobase3_3_17" / "exiobase3_3_17.ttl"
        )
    )
    metadata["activitytype"]["electricity"] = dict(
        get_turtle_labels(
            rdf_base
            / "activitytype"
            / "core"
    ...
    metadata["activitytype"]["entsoe"] = dict(
        get_turtle_labels(rdf_base / "activitytype" / "entsoe" / "entsoe.ttl")
    )
    metadata["flowobject"] = {}
    metadata["flowobject"]["exiobase"] = dict(
        get_turtle_labels(
            rdf_base / "flowobject" / "exiobase3_3_17" / "exiobase3_3_17.ttl"
        )
    )
    metadata["flowobject"]["electricity"] = dict(
        get_turtle_labels(
            rdf_base
            / "flowobject"
            / "core"
    ...
    metadata["location"] = dict(
        get_turtle_labels(
            rdf_base / "location" / "exiobase3_3_17" / "exiobase3_3_17.ttl"
        )
    )
    metadata["unit"] = dict(get_turtle_labels(rdf_base / "unit" / "unit.ttl"))
    metadata["time"] = dict(get_turtle_labels(rdf_base / "time" / "time.ttl"))
