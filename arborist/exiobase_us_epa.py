...
def generate_exiobase_us_epa_uris(output_base_dir):
    ...
    for i in range(0, len(data)):
        ...
        if pandas.notnull(name):
            ...
            g.add(
                (node, OWL.sameAs, URIRef("hhttp://www.chemspider.com/{}".format(nme)))
            )
            ...
            g.add(
                (
                    ...
                    DC.title,
                    Literal(
                        "US EPA Elementary Flow List correspondence with EXIOBASE 3.3.17"
                    ),
                )
            )
            g.add(
            ...
            g.add(
                (node, DC.creator, URIRef("http://bonsai.uno/foaf/bonsai.rdf#bonsai"))
            )
    ...
    with open(
        output_base_dir / "flowobject" / "exiobase3_3_17" / "exiobase3_3_17.ttl", "wb"
    ) as f:
